from advent import ZONE_TIMER
import asyncio
from advent.legacy.models import Zone, LegacyRoom
    ...
    async def update(self):
        ...
        for zid in zone_ids:
            ...
            if timer <= 0:
                ...
                await self.execute_zone_reset(zone)
                await asyncio.sleep(0.5)
                ZONE_TIMER[zid] = zone.lifespan * 60

                for room in zone.rooms.all():
                    if hasattr(room.obj, "at_zone_reset"):
                        room.obj.at_zone_reset()
                        await asyncio.sleep(0)


    async def execute_zone_reset(self, z: Zone):
